...
#include "PLPSLAM/module/loop_detector.h"
#include "PLPSLAM/solve/sim3_solver.h"
...
namespace PLPSLAM
{
    namespace module
    {
        ...
        bool loop_detector::detect_loop_candidates()
        {
            ...
            const keyframe_sets curr_cont_detected_keyfrm_sets = find_continuously_detected_keyframe_sets(cont_detected_keyfrm_sets_, init_loop_candidates);

            // [3] if the number of the detection is equal of greater than the threshold (`min_continuity_ -> 3`),
            ...
            for (auto &curr : curr_cont_detected_keyfrm_sets)
            {
                ...
                const unsigned int continuity = curr.continuity_;
                // check if the number of the detection is equal of greater than the threshold (3)
                                                                              ...
                                                                              const std::vector<data::keyframe *> &keyfrms_to_search) const
        {
            ...
            for (const auto &keyfrm_to_search : keyfrms_to_search)
            {
                ...
                const std::set<data::keyframe *> keyfrm_set = keyfrm_to_search->graph_node_->get_connected_keyframes();
                                                           ...
                                                           std::vector<data::landmark *> &curr_match_lms_observed_in_cand) const
        {
            ...
            for (const auto candidate : loop_candidates)
            {
                ...
                // (FW: transform 2D-2D matching to 2D-3D matching)
                ...
                // estimate the Sim3 using 2D keypoint - 3D landmark matches in linear way
                ...
                // this Sim3 indicates the correct camera pose of the current keyframe after loop correction
