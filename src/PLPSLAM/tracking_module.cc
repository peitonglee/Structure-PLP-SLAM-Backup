...
#include "PLPSLAM/module/local_map_updater.h"
...
namespace PLPSLAM
{
          ...
          keyfrm_inserter_(cfg_->camera_->setup_type_, cfg_->true_depth_thr_, map_db, bow_db, 0, cfg_->camera_->fps_)
    {
        ...
        if (camera_->setup_type_ == camera::setup_type_t::Monocular)
        {
            ...
            ini_extractor_left_->set_max_num_keypoints(ini_extractor_left_->get_max_num_keypoints() * 2); // extract double features if monocular
        }
        ...
        // FW: initialize the line extractor, with camera calibration parameters
        if (map_db_->_b_use_line_tracking)
        {
            _line_extractor = new feature::LineFeatureTracker(camera_);
            if (camera_->setup_type_ == camera::setup_type_t::Stereo)
            {
                _line_extractor_right = new feature::LineFeatureTracker(camera_);
            }
            frame_tracker_.set_using_line_tracking();
        }
    }
    ...
    void tracking_module::set_planar_mapping_module(Planar_Mapping_module *planar_mapper)
    {
    ...
    Mat44_t tracking_module::track_monocular_image(const cv::Mat &img, const double timestamp, const cv::Mat &mask)
    {
        ...
        if (tracking_state_ == tracker_state_t::NotInitialized || tracking_state_ == tracker_state_t::Initializing)
        {
            ...
            {
                // (default)
                curr_frm_ = data::frame(img_gray_, timestamp, ini_extractor_left_, bow_vocab_,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
            else
            {
                // FW: pass the line_extractor to frame.h (case of NotInitialized/Initializing)
                curr_frm_ = data::frame(img_gray_, timestamp, ini_extractor_left_, bow_vocab_,
                                        _line_extractor,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
        ...
        else
        {
            ...
            {
                // (default)
                curr_frm_ = data::frame(img_gray_, timestamp, extractor_left_, bow_vocab_,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
            else
            {
                // FW: pass the line_extractor to frame.h (case of Tracking)
                curr_frm_ = data::frame(img_gray_, timestamp, extractor_left_, bow_vocab_,
                                        _line_extractor,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
    ...
    Mat44_t tracking_module::track_monocular_image(const cv::Mat &img, const cv::Mat &seg_mask_img, const double timestamp, const cv::Mat &mask)
    {
        const auto start = std::chrono::system_clock::now();

        // color conversion
        img.copyTo(img_gray_);
        util::convert_to_grayscale(img_gray_, camera_->color_order_);

        // copy the mask to tracker
        _seg_mask_img = seg_mask_img;

        // create current frame object
        // FW: frame/keyframe only saves keypoints information, however we need to pass planar segmentation mask (as measurement)
        if (tracking_state_ == tracker_state_t::NotInitialized || tracking_state_ == tracker_state_t::Initializing)
        {
            if (!map_db_->_b_use_line_tracking)
            {
                // (default)
                curr_frm_ = data::frame(img_gray_, timestamp, _seg_mask_img, ini_extractor_left_, bow_vocab_,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
            else
            {
        ...
        else
        {
            ...
            {
                // (default)
                curr_frm_ = data::frame(img_gray_, timestamp, _seg_mask_img, extractor_left_, bow_vocab_,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
            else
            {
    ...
    Mat44_t tracking_module::track_stereo_image(const cv::Mat &left_img_rect, const cv::Mat &right_img_rect, const double timestamp, const cv::Mat &mask)
    {
        ...
        if (!map_db_->_b_use_line_tracking)
        {
            // FW: (default)  create current frame object
        ...
        {
            // FW: track stereo frames with LSD/LBD, pass the line_extractor to frame.h
            curr_frm_ = data::frame(img_gray_, right_img_gray, timestamp, extractor_left_, extractor_right_, bow_vocab_,
                                    _line_extractor, _line_extractor_right,
                                    camera_, cfg_->true_depth_thr_, mask);
        }

        left_img_rect.copyTo(curr_frm_._img_rgb);
        track();
    ...
    // Stereo + planar segmentation
    // FW: + line tracker if activated
    Mat44_t tracking_module::track_stereo_image(const cv::Mat &left_img_rect, const cv::Mat &right_img_rect, const cv::Mat &seg_mask_img, const double timestamp, const cv::Mat &mask)
    {
        const auto start = std::chrono::system_clock::now();
        ...
        {
            // FW: (default)  create current frame object
            curr_frm_ = data::frame(img_gray_, right_img_gray, _seg_mask_img, timestamp, extractor_left_, extractor_right_, bow_vocab_,
                                    camera_, cfg_->true_depth_thr_, mask);
        }
        else
        {
    ...
    Mat44_t tracking_module::track_RGBD_image(const cv::Mat &img, const cv::Mat &depthmap, const double timestamp, const cv::Mat &mask)
    {
        ...
        {
            // FW: (default) create current frame object
            curr_frm_ = data::frame(img_gray_, img_depth, timestamp, extractor_left_, bow_vocab_,
                                    camera_, cfg_->true_depth_thr_, mask);
        }
        else
        {
            // FW: track RGB-D frame with LSD/LBD, pass the line_extractor to frame.h
            curr_frm_ = data::frame(img_gray_, img_depth, timestamp, extractor_left_, bow_vocab_,
                                    _line_extractor,
                                    camera_, cfg_->true_depth_thr_, mask);
        }
    ...
    // + line tracker (if activated, see: planar_mapping_parameters.yaml -> Threshold.use_line_tracking)
    Mat44_t tracking_module::track_RGBD_image(const cv::Mat &img, const cv::Mat &depthmap, const cv::Mat &seg_mask_img, const double timestamp, const cv::Mat &mask)
    {
        const auto start = std::chrono::system_clock::now();
        ...
        if (!map_db_->_b_use_line_tracking)
        {
            // (default) create current frame object
            curr_frm_ = data::frame(img_gray_, img_depth, _seg_mask_img, timestamp, extractor_left_, bow_vocab_,
                                    camera_, cfg_->true_depth_thr_, mask);
        }
        else
        {
            // FW: add line_extractor if activated
            curr_frm_ = data::frame(img_gray_, img_depth, _seg_mask_img, timestamp, extractor_left_, bow_vocab_,
                                    _line_extractor,
                                    camera_, cfg_->true_depth_thr_, mask);
        }
    ...
    void tracking_module::track()
    {
        ...
        if (tracking_state_ == tracker_state_t::Initializing)
        {
            ...
            std::vector<data::keyframe *> keyfrms = map_db_->get_all_keyframes();
        ...
        else
        {
            ...
            // again we need to localize our camera against local map to get a better pose
            ...
            if (tracking_state_ == tracker_state_t::Lost && curr_frm_.id_ - initializer_.get_initial_frame_id() < camera_->fps_ * init_retry_thr)
            {
            ...
            // FW: tidy up observations for line
            if (map_db_->_b_use_line_tracking)
            {
                for (unsigned int idx_l = 0; idx_l < curr_frm_._num_keylines; ++idx_l)
                {
                    if (curr_frm_._landmarks_line.at(idx_l) && curr_frm_._outlier_flags_line.at(idx_l))
                    {
                        curr_frm_._landmarks_line.at(idx_l) = nullptr;
                    }
                }
            }
        }
        ...
        // FW: logging for planar mapping module
        if (map_db_->_b_seg_or_not)
        {
            if (_num_planes != map_db_->get_num_landmark_planes())
            {
                // only log when the number of plane changes
                _num_planes = map_db_->get_num_landmark_planes();
                if (_setVerbose)
                    spdlog::info(">tracking< number of Planes in Map currently: {} \n", map_db_->get_num_landmark_planes());
            }
        }
    }
    ...
    bool tracking_module::track_current_frame()
    {
        ...
        {
            if (_setVerbose)
                spdlog::info("Track current frame ({})", curr_frm_.id_);
            ...
            if (!succeeded)
            {
                if (velocity_is_valid_ && last_reloc_frm_id_ + 2 < curr_frm_.id_)
                {
                    ...
                    // NOTE: motion-model based tracking is based on current frame and last frame, they have the same reference frame (keyframe)
                    succeeded = frame_tracker_.motion_based_track(curr_frm_, last_frm_, velocity_);

                    if (succeeded && _setVerbose)
                    {
                        auto after = std::chrono::high_resolution_clock::now();
                        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(after - before).count();
                        spdlog::info("\t \t | process time (motion_based_track): {}ms", duration);
                    }
                }
            }
            if (!succeeded)
            {
                ...
                if (succeeded && _setVerbose)
                {
                    auto after = std::chrono::high_resolution_clock::now();
                    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(after - before).count();
                    spdlog::info("\t \t | process time (bow_match_based_track): {}ms", duration);
                }
            }
            if (!succeeded)
            {
                ...
                if (succeeded && _setVerbose)
                {
                    auto after = std::chrono::high_resolution_clock::now();
                    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(after - before).count();
                    spdlog::info("\t \t | process time (robust_match_based_track): {}ms", duration);
                }
            }
            // FW: if all methods failed, tracking is lost
        }
        else
        {
            // Lost mode
    ...
    void tracking_module::apply_landmark_replace_line()
    {
        for (unsigned int idx = 0; idx < last_frm_._num_keylines; ++idx)
        {
            ...
            auto replaced_lm_line = lm_line->get_replaced();
            if (replaced_lm_line)
            {
                last_frm_._landmarks_line.at(idx) = replaced_lm_line;
            }
        }
    }
    ...
    bool tracking_module::optimize_current_frame_with_local_map()
    {
        ...
        if (map_db_->_b_use_line_tracking)
        {
            // FW: search in parallel
            std::thread thread_searchPoints(&tracking_module::search_local_landmarks, this);
            std::thread thread_searchLines(&tracking_module::search_local_landmarks_line, this);
            thread_searchPoints.join();
            thread_searchLines.join();
        }
        else
        {
            // (default)
        ...
        if (map_db_->_b_use_line_tracking)
        {
            // FW:  motion-only BA with point and line reprojection error
            _pose_optimizer_extended_line.optimize(curr_frm_);
        }
        else
        {
            // (default)
        ...
        // FW: count up the number of tracked 3D lines
        if (map_db_->_b_use_line_tracking)
        {
            _num_tracked_lms_line = 0;
            for (unsigned int idx_l = 0; idx_l < curr_frm_._num_keylines; ++idx_l)
            {
                auto lm_line = curr_frm_._landmarks_line.at(idx_l);
                if (!lm_line)
                {
                    continue;
                }

                if (!curr_frm_._outlier_flags_line.at(idx_l))
                {
                    // the observation has been considered as inlier in the pose optimization
                    assert(lm_line->has_observation());
                    // count up
                    ++_num_tracked_lms_line;
                    // increment the number of tracked frame
                    lm_line->increase_num_observed();
                }
                else
                {
                    // the observation has been considered as outlier in the pose optimization
    ...
    void tracking_module::update_local_map()
    {
        ...
        // FW: clean landmark_line associations
        if (map_db_->_b_use_line_tracking)
        {
            for (unsigned int idx_l = 0; idx_l < curr_frm_._num_keylines; ++idx_l)
            {
        ...
        // FW: inside this function we obtain local landmarks and keyframes
        ...
        // FW: acquire local 3D lines from the map if there is any
        // FW: this way not influence the standard 3D point based pipeline, when there is no 3D line in the map
        if (map_db_->_b_use_line_tracking)
        {
            if (local_map_updater.acquire_local_map_line())
            {
                _local_landmarks_line = local_map_updater.get_local_landmarks_line();
                map_db_->set_local_landmarks_line(_local_landmarks_line);
            }
        }
    }
    ...
    {
        // select the landmarks which can be reprojected from the ones observed in the current frame
        for (auto lm_line : curr_frm_._landmarks_line)
        {
            if (!lm_line)
            {
                continue;
            ...
            lm_line->_identifier_in_local_lm_search = curr_frm_.id_;
        ...
        unsigned int pred_scale_level;
        for (auto lm_line : _local_landmarks_line)
        {
            // avoid the landmarks which cannot be reprojected (== observed in the current frame)
            if (lm_line->_identifier_in_local_lm_search == curr_frm_.id_)
            {
                continue;
            }
            if (lm_line->will_be_erased())
            ...
            // check the observability (we also accept partial occluded line)
            if (curr_frm_.can_observe_line(lm_line, reproj_sp, x_right_sp, reproj_ep, x_right_ep, pred_scale_level))
            {
                // pass the temporary variables
                lm_line->_reproj_in_tracking_sp = reproj_sp;
                lm_line->_reproj_in_tracking_ep = reproj_ep;

                lm_line->_scale_level_in_tracking = pred_scale_level;

                // this landmark can be reprojected
                lm_line->_is_observable_in_tracking = true;
        ...
        const float margin = (curr_frm_.id_ < last_reloc_frm_id_ + 2)
                                 ? 20.0
                                 : 10.0;
        // 局部地图匹配: 没有从上一帧追踪成功的3D线，进一步从局部地图中进行lbd匹配
        projection_matcher.match_frame_and_landmarks_line(curr_frm_, _local_landmarks_line, margin);

        // spdlog::info("search_local_landmarks_line -> num_local_line: {}", _local_landmarks_line.size());
        // spdlog::info("search_local_landmarks_line -> num_matched_line: {}", num_matched_line);
    }
    ...
    void tracking_module::insert_new_keyframe()
    {
        ...
        const auto ref_keyfrm = keyfrm_inserter_.insert_new_keyframe(curr_frm_); // data::keyframe *
